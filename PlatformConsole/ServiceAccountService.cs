...
namespace PlatformConsole
{
    public partial class ServiceAccountService
    {
        ...
        /// </summary>
        private static readonly string ProjectName = "PROJECT_NAME";
        ...
        /// </summary>
        private const string ServiceAccountName = "sa_auther";
        ...
        private static readonly string titleID = "INPUT_TITLE_ID";
        private static readonly string secretKey = "INPUT_SECRET_KEY";
        ...
        {
            //grab refresh token from appdata and set it
            var fileName = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), ".improbable");
            var fileNameBadCode = Path.Combine(fileName, "oauth2");
            var fileNameBadBadBadCode = Path.Combine(fileNameBadCode, "oauth2_refresh_token");
            var refreshTokenInFile = File.ReadAllText(@fileNameBadBadBadCode);
            RefreshToken = refreshTokenInFile;
            ...
            tokenData.Result.Data.TryGetValue("SpatialosPlatformToken", out string token);
        ...
        private static async Task Setup()
        {
            ...
            for (var i = 0; i < NumberOfServiceAccountsToCreate; i++)
            {
                ...
                await PlayFabServerAPI.SetTitleInternalDataAsync(new SetTitleDataRequest() { Key = "SpatialosPlatformToken", Value = resp.Token });
        ...
        private static void Cleanup()
        {
            ...
            for (int i = 0; i < ServiceAccounts.Count; i++)
            {
                ServiceAccount sa = ServiceAccounts[i];
